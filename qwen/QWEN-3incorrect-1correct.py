...
# CONFIG
# =========================
PATENT_FILE = "filtered_A61 - filtered_A61.csv"
QUESTIONS_FILE = "questions_5000.csv"
OUTPUT_FILE = "/content/drive/MyDrive/4patents.csv"
...
def build_four_patent_prompt(
    ...
    return (
        ...
        "Use the patent evidence to answer the multiple-choice question.\n"
        "Do NOT use outside knowledge.\n"
        "Choose the ONE answer choice best supported by the relevant patent evidence.\n\n"
...
patents_unique["similarity_text"] = (
    patents_unique["first_claim"].map(clean_text)
    + " "
    + patents_unique["patent_abstract"].map(clean_text)
)

# Match questions to patents
...
tfidf_matrix = vectorizer.fit_transform(patents_unique["similarity_text"])

patent_id_to_index = {
    pid: idx for idx, pid in enumerate(patents_unique["patent_id"].tolist())
}
...
def get_three_similar_patents(correct_patent_id: str):
    correct_idx = patent_id_to_index[correct_patent_id]
    ...
    for idx, score in enumerate(sims):
        candidate_id = patents_unique.iloc[idx]["patent_id"]

        if candidate_id == correct_patent_id:
            continue
...
def make_four_patent_blocks(correct_patent_id: str):
    similar_patents = get_three_similar_patents(correct_patent_id)

    blocks = []

    # Add 3 similar wrong patents
    ...
    # Add correct patent
    correct_row = patent_id_to_row[correct_patent_id]
    blocks.append({
        "patent_id": correct_patent_id,
        "evidence_text": correct_row["evidence_text"],
        "is_correct_patent": True,
        "similarity_to_correct": 1.0,
    })

    # Shuffle so correct patent is not always Patent 4
    random.shuffle(blocks)

    # Add shown patent number
...
def judge_with_four_patents(row):
    ...
    patent_blocks = make_four_patent_blocks(correct_patent_id)
...
for _, row in tqdm(merged.iterrows(), total=len(merged), desc="Answering with 4 patents"):
    try:
